...
public class Recursion {
	...
	private static int indexOfHelper(String s1, String s2, int index) {
		...
		else if(s1.substring(index).equals("")) {
			return -1; 
		}
		else {
	...
	public static int removeEvenNumbers(Stack<Integer> stack, int k) {
		...
		if(temp%2==0) {
			...
			int result = removeEvenNumbers(stack, k);
			stack.push(temp);
			return result;
		}
	}
	...
	public static int evenDigits(int n) {
		...
		if(n<10) {
			...
			int digit = n%10;
			if(digit%2 == 0) {
				return evenDigits(n/10)*10 + digit;
			}
			else {
	...
	public static int evaluate(Queue<Character> q) {
		...
		char operator; int first, last;
		char c = q.remove();
		
		if(c=='(') {
			first = evaluate(q);
			if(q.peek()==null)   {return first;}
			operator = q.remove();
			...
			if(operator == ')')  {return first;}
			char lastChar = q.remove();
			
			if(lastChar=='(')    {last = evaluate(q);}
			else   {last = Character.getNumericValue(lastChar);}
